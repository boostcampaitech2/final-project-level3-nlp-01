...
# Weigth Distillation 논문에 나온 loss function
class WeightDistillationLoss(nn.Module):
...
# Selective Knowledge Distillation 논문 참고 (실패)
class SelectiveDistillationLoss(nn.Module):
    ...
    def forward(self, teacher_logits, student_logits, labels, top_r, alpha):
        ...
        SelectiveDistillationLoss.queue.extend(torch.tensor(list(zip(labels, loss_ce))))

        while len(SelectiveDistillationLoss.queue) > 50000:
            SelectiveDistillationLoss.queue.popleft()
        
        hard_threshold = int(len(SelectiveDistillationLoss.queue) * top_r) # 0 < top_r <= 1.0
        sorted_queue = sorted(SelectiveDistillationLoss.queue, key=lambda x: x[1], reverse=True)
