...
class GraftAttentionModule(nn.Module):
    def __init__(self, bart_config, model_config, embed_dim):
        ...
        self.graft_encoder = nn.ModuleList(
            [
                BartEncoderLayer(bart_config)
                for _ in range(model_config["num_enc_layer"])
            ]
        )
        self.graft_decoder = nn.ModuleList(
            [
                BartDecoderLayer(bart_config)
                for _ in range(model_config["num_dec_layer"])
            ]
        )
...
class GrafomerModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    def __init__(self, enc_name, dec_name, cfg):
        ...
        self.encoder = getattr(
            __import__("transformers"), cfg["encoder"]["model"]
        ).from_pretrained(enc_name)
        self.decoder = getattr(
            __import__("transformers"), cfg["decoder"]["model"]
        ).from_pretrained(dec_name)
        ...
        self.graft_module = GraftAttentionModule(
            self.bart_config, cfg["graft_module_config"], self.decoder_embed_dim
        )
    ...
    def forward(
        ...
        if decoder_input_ids is not None:
            ...
            dec_mask = _make_causal_mask(decoder_attention_mask.shape, self.dtype).to(
                self.device
            ) + _expand_mask(decoder_attention_mask, self.dtype)
            cross_mask = _expand_mask(
                attention_mask, self.dtype, tgt_len=decoder_input_ids.shape[1]
            )
        ...
        return Seq2SeqLMOutput(logits=output_hidden_states)
